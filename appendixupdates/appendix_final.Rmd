...
### Binomial Tests for Coefficient Sigs

A general test of the theory would be to study whether the coefficients are positive or negative. Note that the law of 1/n poses that we should have a positive influence of legislature size on expenditure. To test this theory, we run a Binomial One-Proportion Z-test. The test consists in contrasting the number of occurences of positive and negative signs (Binomial trials) against the hypothesis that they were produced with 0.5 percent chance. Note that if this is the case, any detected difference would be a mere XXXX of chance.
...
aux <- list(mod3, mod4, mod5, mod6, mod1, mod2) %>%
  map(~{
    left_join(coefs, .x) %>%
  ...
  mutate_at(vars(contains("Estimate") | contains("SE")),
            list(~ifelse(is.na(.), " ", .))) %>%
  mutate(SE = paste0("(", SE,")",`P-Value`)) %>%
  mutate(SE = ifelse(grepl("^\\( ",SE), " ", SE)) %>%
  select(-`T`,-CI,-`P-Value`,-order) %>%
  pivot_longer(cols = c("Estimate", "SE")) %>%
  pivot_wider(names_from = model, values_from = c(value)) %>%
  select(-name)
...
kable(aux, 
  ...
  collapse_rows(1:2,row_group_label_position ='identity',
                latex_hline = "none") %>%
  add_header_above(c(" ","ExpPC" = 2, "logExpPC" = 2, "PCTGDP" = 2)) %>%
  kable_styling(
    c("striped",
      "bordered", 
      "scale_down",
...
aux <- list(mod3_permu, mod4_permu, mod5_permu, mod6_permu, mod1_permu, mod2_permu) %>%
  map(~{
    left_join(coefs, .x) %>%
      mutate(model = ifelse(is.na(model), model[1], model)) 
  }) %>%
