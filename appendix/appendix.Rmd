...
```{r, fig.width=7, fig.height=3, fig.cap="Study Year Frequencies", cache=TRUE, size="footnotesize"}
dat %>%
  select(id, year) %>%
  unique() %>%
...
```{r, fig.width=4, fig.height=3, fig.cap="Was the study published?", size="footnotesize", cache=TRUE}
dat %>%
  select(id, published) %>%
  unique() %>%
...
```{r, fig.width=4, fig.height=3, fig.cap="Electoral Systems", size="footnotesize", cache=TRUE}
dat %>%
  select(id, elecsys2) %>%
  unique() %>%
...
```{r, fig.width=4, fig.height=3, fig.cap="Dependent variables across the law of 1/n studies", size="footnotesize", cache=TRUE}
dat %>%
  select(id, depvar2) %>%
  unique() %>%
...
```{r, fig.width=4, fig.height=3, fig.cap="Independent variables across the law of 1/n studies", size="footnotesize", cache=TRUE}
dat %>%
  select(id, indepvar2) %>%
  unique() %>%
...
```{r, size="footnotesize", cache=TRUE, fig.width=5, fig.height=4}
dat %>%
  ggplot(aes(x = coef)) +
...
```{r, size="footnotesize", fig.width=5, fig.height=4, fig.cap="Coefficient Sign", cache=TRUE}
dat %>%
...
```{r, size="footnotesize", warning=F,message=F, cache=TRUE}
...
```{r, cache=TRUE, size="footnotesize"}
...
```{r, size="footnotesize", warning=F, cache=T}
# Pooling effects analysis -- ExpPC x N
aux <- fulldat %>%
  filter(indepvar2 == 'N',
         depvar2 == 'ExpPC')
...
```{r, size="footnotesize", warning=F, cache=TRUE}
# Pooling effects analysis -- ExpPC x K
aux <- fulldat %>%
  filter(indepvar2 == 'K',
         depvar2 == 'ExpPC')
...
```{r, size="footnotesize", error=F, warning=F, cache=T}
mod <-  rma(yi = coef,
              sei = SE,
              data = dat,
              method = "REML",
...
```{r, size="footnotesize", eval = FALSE, cache=T}
# Build plot function for forest plots
build_forest <- function(mod, capt, lsize = 22, ttl = NULL) {
  # Build dataset for plot
  mod2 <- tibble(
