...
All the remaining papers received a unique index. As the number of the papers with these dependencies is not very high, and many characteristics within papers vary (e. g., state-wise ad-hoc exclusions, start and end points variations, region selection, etc), we show that it makes little difference to change from this index to the paper IDs. In regard to the full dataset, the results change considerably, because authors fit several models within the same paper. In any case, we use multilevel random effects for all the estimated models in this paper.
...
The results for the meta-analysis that compares lower house size (_n_) and expenditure per capita are available below.
...
```{r, fig.width=6, fig.height=4, fig.cap="Effect of log lower houses size (logN) on the log of per capita government expenditure (logExpPC)", echo = FALSE, warning=FALSE, cache=TRUE}
build_forest(mod, NULL)
```
...
Over time, the literature evolved to use causally identified techniques to determine the effect of legislature size on the expenduture per capita. To study whether the method had an effect on the estimated coefficients, we fit a subgroup analysis using the method employed in each paper.
...
```{r, fig.width=4, fig.height=3, fig.cap="Funnel Plot -- Effect of lower houses size (N) on log of Per Capita Expenditure (logExpPC)", warning=FALSE, cache=TRUE, echo = FALSE}
funnel(mod)
```
...
```{r, fig.width=8, fig.height=3, fig.cap="Effect of log lower houses size (logN) on the GDP share of public expenditure (PCTGDP)",warning=FALSE, cache=TRUE, echo = FALSE}
build_forest(mod, NULL)
```
...
With all coefficients, among the results of the effect sizes on the Log of Expenditure Per Capita Regressions, we find that in regard to the estimation method, papers that use `PANEL` data or `RDD`
   designs have smaller effects than articles that employ `OLS` estimations.
...
1. More recent papers find smaller effect sizes than older studies.

2. Papers whose samples included both unicameral and bicameral legislatures report significantly lower coefficients.
...
1. Recent papers find even smaller effect sizes than older studies.

2. Studies using non-majoritarian systems find more positive effects between variables.

3. Papers whose samples included both unicameral and bicameral legislatures report significantly lower coefficients. 
...
summary(mod)
```
In the restricted model, with only the main coefficients in the selected papers, only Unicameralism has a positive, significant effect. We also run the meta-regressions adding all coefficients in the papers. The results follow below:
...
1. Coefficients from non-majoritarian systems are significantly smaller than those found in purely majoritarian systems.

2. Compared with `OLS`, `PANEL`, `IV` and `RDD` report significantly smaller coefficients.

3. Evidence from purely unicameral systems produces significantly larger effect sizes.

4. All other moderators are insignificant.
