...
dat %>%
  ...
  mutate(indepvar2 = factor(indepvar2, 
                            labels = c("K (upper house legislators)",
                                       "Log N",
...
5. A institution design variable, with:
 ...
 - `Non-unicameral`: (e.g. `Bicameral` or `Mixed`).
...
mod <- rma.mv(yi = coef,
              ...
              method = "REML",
              random = ~ 1 | id_level1/id_level2, 
              mods = ~indepvar2+year+published+elecsys2+method+instdesign2,
              test = "knha",
              sparse = TRUE,
              tdist = TRUE,
...
3.  All other moderators were not significant.
...
With all coefficients, results of the effect sizes on the Log of Expenditure Per Capita Regressions indicate that in regard to the estimation method, papers that use `PANEL` data or `RDD` designs have smaller effects than articles that employ `OLS` estimations. All other results are insignificant.
...
1. Coefficients from non-majoritarian systems are significantly lower than those found in purely majoritarian systems.

2. Compared with papers that employ `OLS`, those that use `PANEL`, `IV` and `RDD` report significantly smaller coefficients.

3. Evidence from unicameral systems presents significantly larger effect sizes.
