...
#TABLE 2 ABOUT HERE: Moderator, frequency, percentage (mean and median for publication year)

aux = dat %>%
  group_by(depvar2) %>%
  summarise(n = n(),
            mean_coef = mean(coef, na.rm = T),
            median_coef = median(coef, na.rm = T)) %>%
  mutate(pct = round(n/sum(n, na.rm = T), digits = 2)) %>%
  select(depvar2, n, pct, mean_coef, median_coef)
...
### Descriptive Stats of Moderators by Year
...
aux = dat %>%
  group_by(depvar2, year) %>%
  summarise(n = n(),
            mean_coef = round(mean(coef, na.rm = T), digits = 3),
            median_coef = round(median(coef, na.rm = T), digits = 3)) %>%
  group_by(year) %>%
  mutate(pct = paste0(100*(round(n/sum(n, na.rm = T), digits = 2)), " %")) %>%
  select(depvar2, year, n, pct, mean_coef, median_coef) %>%
  arrange(year) %>%
  rename(Moderator = depvar2,
         Year = year,
         N = n,
         Pct = pct,
         Mean = mean_coef,
...
### Summary of Base Models (Permutation)


```{r,echo=F}

# Table  3  presents  the  meta-regression  results  for  our  restricted  and  extended  samples.   Eachcolumn represents one of the three measures of public spending we discuss in this paper. To reduce the risk of false positives in our analyses, we use permutation tests to calculate significance levelsfor the meta-regressions (Higgins and Thompson 2004).TABLE 3 AROUND HERE: Meta-regression analysis estimating the effect of moderator variables on public expenditure 
#The first two models show the results for public expenditure per capita.  In both the restrictedand the extended samples, we and that models that use푛as an independent variable tend to detect significantly smaller effects when compared to k. This suggests that an additional member in thelower house has a smaller impact on public spending than a member in the upper house. Moreover,
#the results for the extended sample point out that recent studies and larger effects than older ones,and that changing the electoral rules from majoritarian to non-majoritarian increases the effects oflegislature size on per capita expenditure.The third and fourth columns use the logarithm of expenditure per capita as the dependentvariable.   No covariate  is statistically  significant in our  smaller sample,  but two  moderators arenegatively associated with the outcome in our larger study pool.  They both refer to estimationmethods.  Studies that employ panel/fixed effects or regression discontinuity designs (RDDs) havelower coeficients for log expenditure per capita if we take OLS as the reference category
...
### Summary of Models with all coefficients 
...
### Summary of Models with all coefficients (Permutation)
...
aux <- tibble(
  ` ` = c("Intercept", "Depvar: PCTGDP", "Depvar: logExpPC", 
          "Indepvar: N", "Indepvar: logN", "Year", "Published: No",
           "Elecsys: Non-Majoritarian", "Method: PANEL", "Method: IV", "Method: RDD",
          "AggLevel: States", "AggLevel: Countries" ,"Location: World"),
  Estimate = mod[["beta"]],
  SE = mod[["se"]],
  T = mod[["zval"]],
  `P-Value` = mod[["pval"]],
