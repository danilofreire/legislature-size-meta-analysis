...
[^catarina]: PhD Student, University of California San Diego, <catarinamroman@gmail.com>, <http://catarinaroman.github.io>.
...
@gilligan1995deviations; @gilligan2001fiscal]. More recently, the discussion of the "law of $1/n$" has surpassed its original formulation. Specifically, the theory meant to explain distributive behavior of US district legislators under majoritarian electoral rules. It has since then been applied to a number of contexts whose political features do not fit into what @weingast1981political had theorized.

The mechanism behind $1/n$ is incumbents' choice of redistributive strategy to optimize chances of reelection. The _norm of universalism_, by which all legislators pass each other's bills, guarantees that all representatives are allotted an amount of resources to cater projects to their voters. @crowley2019law contests this scenario where coordination costs are minimal, aiming at solving the discrepancies in previous works. The literature finds that a number of players and features can override the $1/n$ effect by posing checks to legislative activity and altering coordination costs. First, shifts in the level of intra or intergovernmental competition based on district homogeneity, supermajority rules, the size of coalitions, and even gerrymandering have been shown to interfere with the effect of chamber size on spending [@bjedov; @crain1999; @crowley2015; @gilligan2006; @lee2015; @mukherjee2003; @pecorino2018]. Second, we must take into account players who function as institutional checks, such as strong executives who take on legislative duties, other deliberative chambers, or spending limits, who also directly influence legislators' access to common pool funds [@bradbury2001legislative; @chen_malhotra_2007; @primo2006stop]. Moreover, @primo2008distributive affirm that, due to spatial spillovers, a
...
insignificant, and negative results. Out 
...
negative and statistically significant. A comprehensive survey and systematic review of the existing evidence may help us reach a
...
lower chamber, its natural logarithm, and the number of members
in the upper chamber. We evaluate their effect upon public expenditure per
...
study specifications. While papers that employ
...
the other measures of public expenditure. However, unicameralism strongly supports the positive effect predicted by the original theory. This result is contradictory to the rest of our findings, even if unicameral cases mostly reflect the very specific framework under which the theory developed (loosely checked local governments). In summary, our analysis does not
...
We collected the study sample using three criteria. First, the study should
cite \citet{weingast1981political} as their work is foundational to the
distributive politics literature. Second, the study had to use quantitative
methods to estimate the relationship between our variables of
interest.\footnote{As meta-analysis requires a single estimate per observation,
we excluded articles that use interaction terms or quadratic specifications of
our independent variables.} Third, the study should be written in the English
language. We conducted a primary search using Scopus, Microsoft Academic, and
Google Scholar, three large academic databases.\footnote{See the Supplementary
Material for code.} Based on the results obtained, we selected key terms to conduct a broader search of the literature. The same eligibility criteria were applied.

Upon restricting the sample, we identified which were the measurement units most commonly used to translate legislature size and public spending. We found those to be public expenditure as a share of GDP, public expenditure per capita, log public expenditure per capita, lower chamber size, upper chamber size, and log lower chamber size. Log upper chamber size is not a commonly used measurement. Unicameralism is captured by lower chamber size ($n = 7$) and by log lower chamber size ($n = 5$). Since much of the literature dedicated to measuring the effects of bicameralism on this relationship, many articles use both lower and upper chamber sizes as key explanatory variables.

After applying our exclusion criteria, the search produced a dataset of 
`r length(unique(dat$id))` studies as of the 5\textsuperscript{th} of March,
2021. Table \ref{tab:papers} contains the full list of articles from which we
drew data. A PRISMA\footnote{The Preferred Reporting Items for Systematic
Reviews and Meta-Analyses (PRISMA) statement provides a checklist designed
to improve the quality and transparency of study selection processes in
meta-analyses. For more information on the PRISMA framework, see
\citet{liberati2009prisma}.} flow diagram showing the number of resulting
papers after each review step is available in section C.2 of the Supplementary Material.
...
addition to the literature, as the first paper to apply it was published in 2012.
...
variables of interest are, lower chamber size (`r as.numeric(table(dat$indepvar2)[which(names(table(dat$indepvar2))=='N')])`), the natural
logarithm of lower chamber size (`r as.numeric(table(dat$indepvar2)[which(names(table(dat$indepvar2))=='logN')])`), and upper chamber
...
indicator for the type of independent variable used in the original study (lower chamber size,
log of lower chamber size, or upper chamber
size). The additional moderators are: 1) publication year; 2) paper
...
To ensure we do not violate the effect size independence assumption, we use two strategies.
Since most selected articles submit their data to a number of tests, varying models and controls, each pair of variables has more than one effect size. First, to reduce the impact of multicollinearity within the same article,
...
ricciuti2003trading}. Our second sample, in contrast, contains all the `r dim(fulldat)[1]` effect sizes reported in the `r length(unique(dat$id))` papers. Here we focus on the results for our restricted sample as we consider them more robust, but the findings are very similar when we use the extended set. We discuss eventual differences in the next section. There is also dependence between studies when more than one article uses the same dataset or source of evidence. In our study, for instance, several papers used US state and municipal data from the Census Bureau and the Book of the States, for the same constituencies, under the same time period. To override dependence between articles, the second strategy we use is employing a multilevel random effects model, grouping the studies that use overlapping data.
...
The first column of the graph reports the results for lower chamber size.
...
this variable and the logarithm of expenditure per capita. The result is positive and
...
measures of public spending we discuss in this paper. Each subgroup uses a chosen category as reference. When it comes to the independent variable, the reference category is upper chamber size; for 
publication bias, the reference is published articles; for electoral system, the reference is majoritarian systems; and for methodology, the reference is ordinary least squares linear regression. To reduce the risk of
