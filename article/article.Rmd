...
our sample. The full list of excluded records is available for online
...
aux <- dat %>%
  select(authoryear, journalCode, locationISO, depvar2, method, instdesign2, elecsys3, year) %>%
  unique() %>%
...
for (i in 1:dim(aux)[1]) {
  cat(paste(build_nams(aux$authoryear[i]), ' & ', aux$journalCode[i], ' & ',
      ...
      aux$method[i], ' & ', aux$instdesign2[i], ' & ', 
...
aux <- select(fulldat, usemeta2, indepvar2, elecsys2, method,
  ...
  rename(`Independent Variables` = indepvar2,
         ...
         `Institutional Design`  = instdesign2,
...
0.299], $p$-value = 0.87), so we cannot rule that the effect is different from
...
[-0.188; 0.127], $p$-value = 0.58), and the impact of larger lower houses on
...
meta-analyses using our extended sample, and the all three coefficients are
...
\toprule
\midrule
\multicolumn{1}{c}{ } & \multicolumn{2}{c}{Expenditure Per Capita} & \multicolumn{2}{c}{Log Expenditure Per Capita} & \multicolumn{2}{c}{Gov. Spending \% GDP} & \multicolumn{2}{c}{Full Model} \\
\cmidrule(l{3pt}r{3pt}){2-3} \cmidrule(l{3pt}r{3pt}){4-5} \cmidrule(l{3pt}r{3pt}){6-7} \cmidrule(l{3pt}r{3pt}){8-9}
& Restricted & Extended & Restricted & Extended & Restricted & Extended & Restricted & Extended \\
\midrule
Independent Variable: log($N$) & 
...
\begin{minipage}{\textwidth}
\renewcommand{\footnoterule}{}
\footnotetext{\textbf{Note:} {The restricted and extended samples include `r length(dat$id)` and `r length(fulldat$id)` study coefficients, respectively. We report the results from the permutation tests. Reference categories: Independent Variable: Upper House Size; Published $= No$; Non-Majoritarian; Method $= OLS$, Legislative: $= Non-Unicameral$. Significance codes: *** $p < 0.01$; ** $p < 0.05$; * $p < 0.10$.}}
\end{minipage}
\end{table}
```
