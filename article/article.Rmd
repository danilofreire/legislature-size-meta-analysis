...
on the "law of $1/n$". We then restricted our query to articles written in the
English language, and to make our study sample comparable, we only selected
papers that use quantitative methods to analyse data\footnote{Since
...
variables. Please refer to Section C in the Supplementary Material for a
detailed description of the selection procedure. We also included two PRISMA
(Preferred Reporting Items for Systematic Reviews and Meta-Analyses) flow
diagrammes \citep{liberati2009prisma} showing the number of resulting papers
after each review step.}. After this stage, we identified the variables that
authors employ more often to measure government expenditure and legislature
size, our dependent and independent variables, respectively. For government
expenditure, our study sample uses (i) public expenditure as a share of GDP;
(ii) public expenditure per capita; and (iii) the natural logarithm of public
...
logarithm of lower chamber size; and (iii) upper house size\footnote{There are
a few important nuances concerning coding of these variables. For instance,
unicameralism is captured both by lower chamber size ($n = 7$) and by log lower
chamber size ($n = 5$). Since much of the literature estimates how
institutional designs affect this relationship, ours and many other articles
use both lower and upper chamber sizes as main explanatory variables. We did
not find any articles that used the natural logarithm of upper chamber size.}.
...
articles written in English which employed quantitative methods. In the third
search round, we looked into the personal webpages of every author we had
already included in our previous sample. The purpose of this manual search was
to assess whether there was any working paper or unpublished manuscript which
we might had missed in the previous queries. All papers we found in the last
search had already been included in our sample. Combined, the three searches
...
sample, authors frequently use similar datasets in their analyses,
and almost all papers fit more than one model with similar variables, what
suggests that the assumption does not hold. We use two procedures to tackle this
